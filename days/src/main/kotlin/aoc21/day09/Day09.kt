...
import utils.konsole.Color
import utils.konsole.Intensity
import utils.konsole.format
import utils.konsole.reset
import java.io.File
...
fun printState(basinMap: Heightmap) {
    ...
    File("src/main/resources/day09/frames/heights_$stamp.csv").bufferedWriter().use { writer ->
        basinMap.forEach {
...
data class Point(val i: Int, val j: Int, val map: Heightmap) {
    ...
    fun expand(basinMap: Heightmap, basinID: Int) {
        File("src/main/resources/day09/frames/path.txt").appendText("$i,$j\n")
